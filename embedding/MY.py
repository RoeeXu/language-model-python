...
def my_sru(data, nh, ni, bs, ts):
    ...
    #W = tf.Variable(tf.random_uniform([ni, nh], -limit_u, limit_u))
    #F = tf.Variable(tf.random_uniform([bs, ts, nh], -limit_u, limit_u))
    #V = tf.Variable(tf.random_uniform([nh, nh], -limit_u, limit_u))
    #f = tf.unstack(F, ts, 1)
    ...
    #    if pos == 0:
    #        c[0] = tf.matmul(x[0], W)
    #        s[0] = tf.matmul(tf.sigmoid(c[0]), V)
    #    else:
    #        c[pos] = f[pos] * c[pos - 1] + tf.matmul(x[pos], W)
    #        s[pos] = tf.matmul(tf.sigmoid(c[pos]), V)
    #o = tf.stack(s, 1)
    W = tf.Variable(tf.random_uniform([ni, nh], -limit_u, limit_u))
    o = tf.einsum('ijk,kl>ijl', data, W)
    return o
