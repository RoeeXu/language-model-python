...
def LM(embeddings, X):
    ...
    o = my_rnn(x, OUT_SIZE, EMBED_SIZE, BATCH_SIZE, WIN_NUM)
    return o
...
if __name__ == "__main__":
    ...
    for i in range(EPOCH_LEN):
        x = [dat[:, i*num_steps:(i+1)*num_steps], dat[:, i*num_steps+2:(i+1)*num_steps+2]]
    ...
    data_len = len(test)
    batch_len = data_len // BATCH_SIZE
    epoch_len = (batch_len - 2) // num_steps
    dat = np.zeros([BATCH_SIZE, batch_len], dtype=np.int32)
    for i in range(BATCH_SIZE):
        dat[i] = data[batch_len * i:batch_len * (i+1)]
    test_data = []
    ...
    #===Train============================================
    X1 = tf.placeholder(tf.int32, shape=[BATCH_SIZE, WIN_NUM])
    X2 = tf.placeholder(tf.int32, shape=[BATCH_SIZE, WIN_NUM])
    ...
    O1 = tf.reshape(LM(embeddings, X1), [-1, OUT_SIZE])
    
    temp1 = tf.unstack(X2, WIN_NUM, 1)
    temp2 = [temp1[len(temp1)-i-1] for i in range(len(temp1))]
    X2_T = tf.stack(temp2, 1)
    O2_T = LM(embeddings, X2_T)
    temp3 = tf.unstack(O2_T, WIN_NUM, 1)
    temp4 = [temp3[len(temp3)-i-1] for i in range(len(temp3))]
    O2 = tf.reshape(tf.stack(temp4, 1), [-1, OUT_SIZE])
    O = tf.concat([O1, O2], 1)
    ...
    logits = tf.matmul(O, tf.transpose(loss_weights)) + loss_biases
    ...
    for i in range(TRAIN_STEPS+1):
        ...
        _, p = sess.run([optimizer, logppl], feed_dict={X1: batch[0], X2: batch[1], Y: label})
        if (i%BATCH_LEN)%(BATCH_LEN//20) == 0:
            ppl = np.exp(p / len(train_data[0][0][0][0]))
            costs = 0
            for batch, label in valid_data:
                cost = sess.run(logppl, feed_dict={X1: batch[0], X2: batch[1], Y: label})
                costs += cost
            valid_ppl = np.exp(costs / len(valid_data) / len(valid_data[0][0][0][0]))
            costs = 0
            for batch, label in test_data:
                cost = sess.run(logppl, feed_dict={X1: batch[0], X2: batch[1], Y: label})
                costs += cost
            test_ppl = np.exp(costs / len(test_data) / len(test_data[0][0][0][0]))
