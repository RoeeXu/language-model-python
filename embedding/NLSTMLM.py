...
def my_nlstm(data, nh, ni, bs, ts):
    ...
    new_i_x = tf.Variable(tf.random_uniform([ni, nh], -limit_u, limit_u))
    new_i_s = tf.Variable(tf.random_uniform([nh, nh], -limit_v, limit_v))
    new_i_b = tf.Variable(tf.zeros([nh]))
    new_f_x = tf.Variable(tf.random_uniform([ni, nh], -limit_u, limit_u))
    new_f_s = tf.Variable(tf.random_uniform([nh, nh], -limit_v, limit_v))
    new_f_b = tf.Variable(tf.zeros([nh]))
    new_c_x = tf.Variable(tf.random_uniform([ni, nh], -limit_u, limit_u))
    new_c_s = tf.Variable(tf.random_uniform([nh, nh], -limit_v, limit_v))
    new_c_b = tf.Variable(tf.zeros([nh]))
    new_o_x = tf.Variable(tf.random_uniform([ni, nh], -limit_u, limit_u))
    new_o_s = tf.Variable(tf.random_uniform([nh, nh], -limit_v, limit_v))
    new_o_b = tf.Variable(tf.zeros([nh]))
    
    sm = tf.zeros([bs, ts, nh])
    cm = tf.zeros([bs, ts, nh])
    ncm = tf.zeros([bs, ts, nh])
    s = tf.unstack(sm, ts, 1)
    c = tf.unstack(cm, ts, 1)
    new_c = tf.unstack(ncm, ts, 1)
    x = tf.unstack(data, ts, 1)

    for pos in range(ts):
        if pos == 0:
            new_x = tf.sigmoid(tf.matmul(x[0], i_x) + i_b)
            new_c[0] = tf.sigmoid(tf.matmul(new_x, new_i_x) + new_i_b) * tf.tanh(tf.matmul(new_x,
                        new_c_x) + new_c_b)
            new_h = tf.sigmoid(tf.matmul(new_x, new_o_x) + new_o_b) * tf.tanh(new_c[0])
            c[0] = new_h
            s[0] = tf.sigmoid(tf.matmul(x[0], o_x) + o_b) * tf.tanh(c[0])
        else:
            new_x = tf.sigmoid(tf.matmul(x[pos], i_x) + tf.matmul(s[pos-1], i_s) + i_b) * tf.tanh(tf.matmul(x[pos], c_x) + tf.matmul(s[pos-1], c_s) + c_b)
            temp_h = tf.sigmoid(tf.matmul(x[pos], f_x) + tf.matmul(s[pos-1], f_s) + f_b) * c[pos-1]
            new_c[pos] = tf.sigmoid(tf.matmul(new_x, new_f_x) + tf.matmul(temp_h, new_f_s) + new_f_b) * new_c[pos-1] + tf.sigmoid(tf.matmul(new_x, new_i_x) + tf.matmul(temp_h, new_i_s) + new_i_b) * tf.tanh(tf.matmul(new_x, new_c_x) + tf.matmul(temp_h, new_c_s) + new_c_b)
            new_h = tf.sigmoid(tf.matmul(new_x, new_o_x) + tf.matmul(temp_h, new_o_s) + new_o_b) * tf.tanh(new_c[pos])
            c[pos] = new_h
            s[pos] = tf.sigmoid(tf.matmul(x[pos], o_x) + tf.matmul(s[pos-1], o_s) + o_b) * tf.tanh(c[pos])
    o = tf.stack(s, 1)
    return o
...
def LM(embeddings, X):
    ...
    o = my_nlstm(x, OUT_SIZE, EMBED_SIZE, BATCH_SIZE, WIN_NUM)
    return o
